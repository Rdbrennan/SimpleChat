...
class UserInfoPhoneNumberTableViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        if #available(iOS 11.0, *) {
            copy1.rightAnchor.constraint(equalTo: safeAreaLayoutGuide.rightAnchor, constant: -15).isActive = true
            add.rightAnchor.constraint(equalTo: safeAreaLayoutGuide.rightAnchor, constant: -15).isActive = true
        } else {
            copy1.rightAnchor.constraint(equalTo: rightAnchor, constant: -15).isActive = true
        ...
        copy1.widthAnchor.constraint(equalToConstant: 20).isActive = true
        copy1.heightAnchor.constraint(equalToConstant: 20).isActive = true
        copy1.centerYAnchor.constraint(equalTo: phoneLabel.centerYAnchor, constant: 0).isActive = true
        
        add.addTarget(self, action: #selector(handleAddNewContact), for: .touchUpInside)
        copy1.addTarget(self, action: #selector(handleCopy), for: .touchUpInside)
        copy1.setImage(copyNumberImage, for: .normal)
