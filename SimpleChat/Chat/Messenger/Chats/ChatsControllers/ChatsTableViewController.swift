...
//        navigationController?.pushViewController(destination, animated: true)
    ...
    func initialWeclomeMessage2() {
        ...
        let userObject = [
            key:[
                ...
                "seen":             false,
                "status":           "Delivered",
                "text":             "Hello \(name)!\n\n I am test chatbot  here to show you the functionality of adding a new message.",
                "timestamp":        timestamp,
                "toId":             "\(uid)",
    ...
    fileprivate func updateDefaultChat(with conversation: Conversation ) {
        ...
        print(userID)
        userReference = Database.database().reference().child("users").child(userID).child("profile")
